...
try:
    ...
    st.error("❌ 未找到模型文件 `risk_model.pkl`，请确保该文件存在于当前目录。")
    st.stop()
...
if uploaded_file is not None:
    ...
    with st.expander("🧑‍💼 单企业风险评分查询", expanded=False):
        ...
        selected_id = st.selectbox("选择企业（索引或ID）", id_list, format_func=lambda x: f"企业 {x}")
        if 'id' in df.columns:
        ...
        st.write("**关键特征**:", {k: v for k, v in row[feature_cols].items() if not pd.isna(v)})
    ...
    with st.expander("🏦 多银行主体融资关联图谱", expanded=False):
        if 'bank_count' in df.columns:
            ...
            if G.number_of_nodes() > 0:
                ...
                fig4.update_layout(title='企业-银行关联示意图（前20条）', showlegend=False, hovermode='closest')
                st.plotly_chart(fig4, use_container_width=True)
            else:
                st.info("无关联边可绘制")
        else:
            st.warning("数据中无 'bank_count' 字段，无法绘制关联图谱")

else:
    st.info("👈 请从左侧上传 CSV 数据文件开始分析")
    st.markdown("""
    ### 📌 使用说明
    1. 点击左侧 **“上传企业融资数据”** 按钮，上传 CSV 文件。
    2. 系统自动进行风险预测，展示概率分布和等级。
    3. 可查看 **单企业评分**、**行业热力图** 和 **银企关联图谱**。
    4. 一键导出高风险企业名单。
    """)